...
- [x] **Phase 10 optimization** — caching (unstable_cache for master data, tagged revalidation), security headers (CSP, HSTS, X-Frame-Options), AVIF/WebP images, ISR invalidation wired to admin approve/reject/edit, loading.tsx skeleton screens
- [x] **Phase 11 testing** — Vitest unit tests (95 tests): formatters + Zod schemas; Playwright E2E smoke suite (9 tests); GET /api/v1/health endpoint; test:coverage + test:e2e scripts
- [x] **Phase 12 deployment** — GitHub Actions CI (Node 22, unit tests + build gate); Suspense boundaries for useSearchParams (auth pages + UsersFilter); CRON_SECRET + NEXT_PUBLIC_SITE_URL added to Vercel; production deploy
