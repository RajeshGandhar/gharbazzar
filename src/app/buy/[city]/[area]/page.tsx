...
async function getAreaWithProperties(citySlug: string, areaSlug: string, page = 1, perPage = 20) {
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const result = await getAreaWithProperties(citySlug, areaSlug);
  if (!result) return { title: "Area not found" };
  const { city, area } = result;

  return {
    title: `Property for Sale in ${area.name}, ${city.name} | GharBazaar`,
    description: `Browse verified properties for sale in ${area.name}, ${city.name}. Apartments, plots, independent houses — real prices, direct contact on GharBazaar.`,
    alternates: { canonical: `/buy/${city.slug}/${area.slug}` },
    openGraph: { title: `Property for Sale in ${area.name}, ${city.name}`, type: "website" },
  };
}

export default async function BuyAreaPage({ params, searchParams }: Props) {
  const { city: citySlug, area: areaSlug } = await params;
  const { page: pageStr } = await searchParams;
  const page = Math.max(1, parseInt(pageStr ?? "1", 10) || 1);
  const perPage = 20;

  const result = await getAreaWithProperties(citySlug, areaSlug, page, perPage);
  if (!result) notFound();
  ...
  const breadcrumbItems = [
    ...
    { name: area.name, url: `/buy/${city.slug}/${area.slug}` },
  ];
  ...
  const paginationBase = `/buy/${city.slug}/${area.slug}?`;
  ...
  return (
    <>
      ...
      {listingSchemaItems.length > 0 && (
        <script
          type="application/ld+json"
          dangerouslySetInnerHTML={{ __html: JSON.stringify(itemListSchema(listingSchemaItems, `Properties for Sale in ${area.name}`)) }}
        />
      )}
        ...
        <div className="border-b border-white/[0.06]">
          <nav aria-label="Breadcrumb" className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-3">
            <ol className="flex items-center gap-1.5 text-sm text-muted-foreground flex-wrap">
              ...
              <li aria-hidden><ChevronRight className="h-3.5 w-3.5" /></li>
              <li><Link href={`/buy/${city.slug}`} className="hover:text-primary transition-smooth">{city.name}</Link></li>
              <li aria-hidden><ChevronRight className="h-3.5 w-3.5" /></li>
        ...
        <section className="border-b border-white/[0.06] py-10">
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            ...
            <p className="text-muted-foreground max-w-2xl">
              {count > 0
                ? `${count.toLocaleString("en-IN")} verified listings in ${area.name}, ${city.name}${priceMin ? ` · from ${formatPrice(priceMin)}` : ""}${priceMax ? ` to ${formatPrice(priceMax)}` : ""}`
                : `Listings in ${area.name} coming soon — save a search to be notified.`}
            </p>
          </div>
        </section>
        ...
        <div className="mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-8 py-8">
          {properties.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-24 text-center gap-4">
              ...
              <h2 className="text-xl font-semibold">No properties found in {area.name}</h2>
              <p className="text-muted-foreground text-sm max-w-sm">
                Save a search alert and we&apos;ll notify you when listings go live here.
              </p>
              <Link href={`/buy/${city.slug}`} className={buttonVariants({ variant: "outline" })}>
                Browse all of {city.name}
              </Link>
            </div>
          ) : (
            <>
          ...
          <div className="mt-12 border-t border-white/[0.06] pt-8">
            <Link href={`/buy/${city.slug}`} className="text-sm font-medium text-primary hover:text-primary/80">
              ← All properties for sale in {city.name}
            </Link>
          </div>
        </div>
      </div>
